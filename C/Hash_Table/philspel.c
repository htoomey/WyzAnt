...
unsigned int stringHash(void *s) {
  ...
  // -- TODO --
	int i = 0;
        ...
        return i;
        fprintf(stderr,"Define a stringHash\n");
        exit(0);
	...
	FILE * dict;
        ...
        if(dict!=NULL){
                ...
                while(fscanf(dict, "%s", word) != EOF){
                        char *k = (char *) malloc((strlen(word)+1)*sizeof(char));
                        strcpy(k, word);
                        insertData(dictionary,k,k);
                }
        }
        else {
...
//while(c != EOF || temp != NULL){
  while (0) {
    ...
    if ((c >= 65 && c <= 90) || (c >= 97 && c <= 122)) {
      ...
      if (findData(dictionary, temp) != NULL) { // if the word is in dict
        printf("%s ", temp);
        free(temp);
        i = 0;
