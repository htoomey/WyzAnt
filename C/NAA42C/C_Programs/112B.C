/******************************************************************************
	    Nonlinear Shooting - Algorithm 11.2B  (Secant Method)
*******************************************************************************
...
Exercise Set 11.2, Problems *** 4 ***, 5.
...
char *outfile = "112b.out";	/* Customized default output file name.     */
				/* Needs updating  $  */
...
main()
{
  ...
  double a, b, alpha, beta, h, x, TK, TOL, T0, T1, T2, w1oldN;
  int i, kk, N, ch, M;
  ...
  printf2("Nonlinear Shooting - Algorithm 11.2B  (Secant Method)\n\n");
  printf2("%s\n", eq_text_y2prime);
  printf2("%s\n", eq_text_fy);
  printf2("%s\n", eq_text_fyprime);
  printf2("%s\n\n", eq_text_y);
...
*/
    ...
    /* STEP #10 */
    /* Secant method is used to compute TK. */
    if (kk == 1) {
      T0 = TK;
      T1 = T0 + (beta - w[1][N])/(b - a);
      TK = T1;
    } else {
      T2 = T1 - (w[1][N] - beta)*(T1 - T0)/(w[1][N] - w1oldN);
      TK = T2;
      T0 = T1;
      T1 = T2;
    }
