/******************************************************************************
		Successive Over Relaxation (SOR) - Algorithm 7.3
*******************************************************************************

To solve AX = B given the parameter w and an initial approximation X0:
...
the parameter w; tolerance TOL; maximum number of iterations N.
...
char *outfile = "073.out";	/* Customized default output file name.     */
int n;				/* Number of equations and unknowns.        */


main()
{
  double w, **A, *X, *B, *X0, *TEMP, TOL, infinite_norm();
  int i, j, k, N;
  ...
  printf2("Successive Over Relaxation (SOR) - Algorithm 7.3\n\n");

  printf("Enter the Relaxation-Factor, w: ");
  scanf("%lf", &w);
  fprintf(file_id, "Relaxation-Factor w =  %lg\n", w);
  ...
  while (k <= N) {		/* Do Steps 3-6. */
    ...
    for (i=1;i<=n;i++) {
      ...
      for (j=i+1;j<=n;j++)
        X[i] -= A[i][j] * X0[j];
      X[i] = (1.0 - w)*X0[i] + (w*(X[i] + B[i]) / A[i][i]);
    }
