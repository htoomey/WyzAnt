...
Exercise Set 6.2, Problem 11.  For chopping arithmetic, replace EVERY
...
char *outfile = "061d2.out";	/* Customized default output file name.     */
...
main()
{
  ...
  for (i=1;i<n;i++) {		/* Do Steps 2-4.  Elimination process. */
    ...
    for (p=i;p<=n;p++) {	/* Let p be the smallest integer with   */
      if (A[p][i] != 0.0) {	/* i ó p ó n and a(p(i)) != 0.          */
        FLAG = TRUE;
        break;
      }
  ...
  */
  ...
  for (j=n;j>=2;j--) {
    for (i=1;i<j;i++) {
      temp = round(A[j][j] / A[i][j], SIG);
      for (k=1;k<=n+1;k++)
        E[i][k] = round(round(-temp * E[i][k], SIG) + E[j][k], SIG);
    }
    printf2("At column %d:\n", j);
    printf2("E = ");
    print_matrix(E,n);		/* Show intermediate results for looks. */
