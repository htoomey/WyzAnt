/******************************************************************************
	    Nonlinear Shooting - Algorithm 11.2C (Newton's Method)
     (Using Adams-Bashforth and Adams-Moulton Predictor-Corrector Method)
*******************************************************************************
...
char *outfile = "112c.out";	/* Customized default output file name.     */
				/* Needs updating  $  */
...
main()
{
  ...
  double a, b, alpha, beta, h, x, TK, TOL, tmp;
  int i, j, kk, N, ch, M;
  ...
  printf2("Nonlinear Shooting - Algorithm 11.2C (Newton's Method)\n");
  printf2("Using Adams-Bashforth and Adams-Moulton Predictor-Corrector ");
  printf2("Method\n\n");
  printf2("%s\n", eq_text_y2prime);
  printf2("%s\n", eq_text_fy);
  printf2("%s\n", eq_text_fyprime);
  ...
  */
  ...
  while (kk <= M) {		/* Do Steps 3-10. */
    ...
    /* STEP #4 */	/* MODIFICATION #1 - Adjust for loop to 1...3 */
    for (i=1;i<=3;i++) {	/* Do Steps 5-6. */
...
*/
      K[2][2] = h * (fy(x+0.5*h,w[1][i-1],w[2][i-1])*(u1+0.5*K[1][1])
		 + fyprime(x+0.5*h,w[1][i-1],w[2][i-1])*(u2
                 + 0.5*K[1][2]));
      K[3][1] = h * (u2 + 0.5*K[2][2]);
      K[3][2] = h * (fy(x+0.5*h,w[1][i-1],w[2][i-1])*(u1
                 + 0.5*K[2][1])
		 + fyprime(x+0.5*h,w[1][i-1],w[2][i-1])*(u2
                 + 0.5*K[2][2]));
      K[4][1] = h * (u2 + K[3][2]);
    ...
    /*
    ** MODIFICATION #2 - Add the Adams-Bashforth and Adams-Moulton 
    **                   Predictor-Corrector Method here.  See
    **                   Algorithm 5.4 (p. 266).
    */
    ...
    for (i=4;i<=N;i++) {
      ...
      /* Using Adams-Bashforth Four-Step Method (Eq. 5.63) to predict w. */
      w[1][i] = w[1][i-1] + h*(55.0*w[2][i-1] - 59.0*w[2][i-2]
                + 37.0*w[2][i-3] -  9.0*w[2][i-4])/24.0;

      w[2][i] = w[2][i-1] + h*(55.0*f(x-h,  w[1][i-1],w[2][i-1])
                - 59.0*f(x-2*h,w[1][i-2],w[2][i-2])
                + 37.0*f(x-3*h,w[1][i-3],w[2][i-3])
                -  9.0*f(x-4*h,w[1][i-4],w[2][i-4]))/24.0;

      /* Using Adams-Moulton Three-Step Method (Eq. 5.68) to correct w. */
      w[1][i] = w[1][i-1] + h*(9.0*w[2][i] + 19.0*w[2][i-1]
                -  5.0*w[2][i-2] + w[2][i-3])/24.0;

      w[2][i] = w[2][i-1] + h*(9.0*f(x,w[1][i],w[2][i])
                + 19.0*f(x-h,  w[1][i-1],w[2][i-1])
                -  5.0*f(x-2*h,w[1][i-2],w[2][i-2])
                +      f(x-3*h,w[1][i-3],w[2][i-3]))/24.0;


      /* Using Adams-Bashforth Four-Step Method (Eq. 5.63) to predict u. */
      u1 = u1 + h*u2;

      tmp = 55.0*(fy(x-h,  w[1][i-1],w[2][i-1]) * u1 +
             fyprime(x-h,  w[1][i-1],w[2][i-1]) * u2)
          - 59.0*(fy(x-2*h,w[1][i-2],w[2][i-2]) * u1 +
             fyprime(x-2*h,w[1][i-2],w[2][i-2]) * u2);

      u2 = u2 + h*(tmp + 37.0*(fy(x-3*h,w[1][i-3],w[2][i-3]) * u1 +
                          fyprime(x-3*h,w[1][i-3],w[2][i-3]) * u2)
                       -  9.0*(fy(x-4*h,w[1][i-4],w[2][i-4]) * u1 +
                          fyprime(x-4*h,w[1][i-4],w[2][i-4]) * u2))/24.0;

      /* Using Adams-Moulton Three-Step Method (Eq. 5.68) to correct u. */
      u1 = u1 + h*u2;

      tmp =  9.0*(fy(x,  w[1][i],  w[2][i])   * u1 +
             fyprime(x,  w[1][i],  w[2][i])   * u2)
          + 19.0*(fy(x-h,w[1][i-1],w[2][i-1]) * u1 +
             fyprime(x-h,w[1][i-1],w[2][i-1]) * u2);

      u2 = u2 + h*(tmp - 5.0*(fy(x-2*h,w[1][i-2],w[2][i-2]) * u1 +
                         fyprime(x-2*h,w[1][i-2],w[2][i-2]) * u2)
                       +     (fy(x-3*h,w[1][i-3],w[2][i-3]) * u1 +
                         fyprime(x-3*h,w[1][i-3],w[2][i-3]) * u2))/24.0;
    }
