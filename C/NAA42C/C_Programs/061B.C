...
Exercise Set 6.2, Problems 1, 3.  Also see p. 334, Exercise Set 6.3, Problems
1 (iii), 3-4.  For chopping arithmetic, replace EVERY occurance of the word
"round" with "trunc", including the "round.c" file.
...
char *outfile = "061b.out";	/* Customized default output file name.     */


main()
{
  float **A, **E, **m, *X, sum, temp;
  ...
  printf2("Gaussian Elimination with Backward Substitution");
  printf2(" - Algorithm 6.1B\n\n");
  ...
  for (i=1;i<=n;i++)
    for (j=1;j<=n;j++) {
      printf("\tA[%d][%d] = ", i, j);
      scanf("%f", &A[i][j]);
      A[i][j] = round(A[i][j], SIG);	/* Round inputs */
    }
  ...
  for (i=1;i<=n;i++) {
    printf("\tB[%d] = ", i);
    scanf("%f", &E[i][n+1]);
    E[i][n+1] = round(E[i][n+1], SIG);	/* Round inputs */
  }
  printf("\n");
  ...
  /* STEP #8 */
  X[n] = round(E[n][n+1] / A[n][n], SIG); /* Start backward substitution. */

  /* STEP #9 */
  for (i=n-1;i>=1;i--) {
    sum = 0.0;
    for (j=i+1;j<=n;j++) {
      sum += round(A[i][j] * X[j], SIG);
      sum = round(sum, SIG);
    }
    X[i] = round(round(E[i][n+1] - sum, SIG) / A[i][i], SIG);
  }
