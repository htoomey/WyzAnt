...
NOTE: This method has serious stability problems.

NOTE: This algorithm was included as a "Homework Helper."  See p. 632,
Exercise Set 12.3, Problem *** 6 ***.
...
char *outfile = "122c.out";	/* Customized default output file name.     */
char *eq_text_f = "f(x)   = sin(PI*x)";	/* Needs updating  $  */

double *w, *wprev, *wprev2, f();
...
main()
{
  ...
  printf2("Heat Equation Richardson's Method - Algorithm 12.2C\n\n");
  ...
  w      = dvector(0,m);
  wprev  = dvector(0,m);	/* Solution at t(i-1). */
  wprev2 = dvector(0,m);	/* Solution at t(i-2). */
  ...
  for (i=0;i<=m;i++) {		/* Initial values.                           */
    wprev2[i] = f(i*h);		/* May be incorrect, but close, since we do  */
				/* not know the initial conditions at t(-1). */
    wprev[i] = f(i*h);
  }
...
int i;
{
  if ((i == 0) || (i == m))	/* Boudary conditions. */
    ...
    return (2*lambda * (wprev[i+1] - 2*wprev[i] + wprev[i-1])
	+ wprev2[i]);
}
