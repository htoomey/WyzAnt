/******************************************************************************
		Aitken's Iterated Interpolation - Algorithm 3.1C
*******************************************************************************
...
Exercise Set 3.3, Problems 8-11.
...
char *outfile   = "031c.out";	/* Customized default output file name.     */
...
main()
{
  ...
  printf2("Aitken's Iterated Interpolation - Algorithm 3.1C\n\n");
  ...
  for (i=1;i<=n;i++)
    for (j=1;j<=i;j++)
      Q[i][j] = ((X-x[j-1])*Q[i][j-1] - (X-x[i])*Q[j-1][j-1])
		/ (x[i] - x[j-1]);	/* Aitken's Method. */
