/******************************************************************************
		Milne-Simpson Predictor-Corrector - Algorithm 5.4C
*******************************************************************************
...
char *outfile   = "054c.out";	/* Customized default output file name.     */
...
main()
{
  ...
  double f(), y(), K1, K2, K3, K4, val2, val3;
  int i, j, ch, N;
  ...
  printf2("Milne-Simpson Predictor-Corrector - Algorithm 5.4C\n\n");
  ...
  /* Compute starting values using Runge-Kutta methods. */
  ...
  for (i=4;i<=N;i++) {		/* Do Steps 7-10. */
    ...
    W = w[0] + (4*h/3)*(2*val3 - val2 + 2*f(t[1],w[1]));
				/* Predict w(i). */
    W = w[2] + (h/3)*(f(T,W) + 4*val3 + val2);
				/* Correct w(i). */
    ...
    /* STEP #9 */
    for (j=0;j<=2;j++) {
      t[j] = t[j+1];		/* Prepare for next iteration. */
      w[j] = w[j+1];
    }
