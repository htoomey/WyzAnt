...
the numbers x0 < x1 < ... < xn, satisfying S'(x0) = f'(x0) and S'(xn) = f'(xn):

INPUT n; x0,x1,...,xn; either generate a(i) = f(x(i)) for i = 0,1,...,n or
input a(i) for i = 1,...,n; FPO = f'(x0); FPN = f'(xn).
...
char *outfile   = "035.out";	/* Customized default output file name.     */
...
main()
{
  double *a, *alpha, *b, *c, *d, *h, *l, *mu, *x, *z, FPO, FPN, f();
  ...
  printf2("Clamped Cubic Spline - Algorithm 3.5\n\n");
  ...
  fprintf(file_id,
    "Computes %d coefficient sets for a Clamped Cubic Spline.\n\n", n);
  ...
  printf("Do the a(i) = f(x(i)) need to be generated");
  printf(" from f(x)? (Y/N) Y: ");
  ...
  for (i=0;i<=n;i++) {		/* Enter x(i). */
    ...
    if (ch == 'n') {		/* Input a(i). */
      ...
      a[i] = f(x[i]);		/* Use function f(x(i)) for a(i).      */
				...
				/* Enter derivatives at the endpoints. */
  printf("Enter FPO = f'(%lg): ", x[0]);
  scanf("%lf", &FPO);
  printf("Enter FPN = f'(%lg): ", x[n]);
  scanf("%lf", &FPN);
  ...
  printf2("FPO = f'(%lg) = %.9lg\n", x[0], FPO);
  printf2("FPN = f'(%lg) = %.9lg\n", x[n], FPN);

				/* Print table header. */
  printf2("\n i\tx(i)\t a(i) = f(x)\t b(i)\t\t c(i)\t\t d(i)\n");
  printf2("---------------------------------------");
  printf2("----------------------------------------\n");
  ...
  /* STEP #4 */
  l[0]  = 2.0*h[0];		/* Steps 4, 5, 6, and part of step 7 solve   */
  mu[0] = 0.5;			/* a tridiagonal system using Algorithm 6.7. */
  z[0]  = alpha[0] / l[0];
