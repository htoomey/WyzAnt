...
by factoring A into LU, solving LZ = B and UX = Z where the main diagonal of
...
char *outfile = "064c.out";	/* Customized default output file name.     */


main()
{
  float **A, **E, **U, **L, *X, *Z, max, sum, temp, HOLD;
  int i, j, k, n, p, ch;
  ...
  printf2("Direct Factorization with Maximal Column Pivoting");
  printf2(" - Algorithm 6.4C\n\n");
  ...
  /* STEP #1 */
  max = 0.0;
  for (j=1;j<=n;j++) {		/* Find maximum element in column i. */
    temp = fabs(A[j][1]);
    if (temp > max) {
      max = temp;
      p = j;
  ...
  /* STEP #2 */
  if (p != 1)			/* Interchange rows p and 1 in A. */
    for (k=1;k<=n+1;k++) {
      temp    = E[p][k];
      E[p][k] = E[1][k];
      E[1][k] = temp;
  ...
  for (i=2;i<n;i++) {
    ...
    /* STEP #7 */
    if (p != i)			/* Interchange rows p and i in A and L.     */
      for (k=1;k<=n+1;k++) {
        temp    = E[p][k];	/* Matrix A.                                */
        E[p][k] = E[i][k];
        E[i][k] = temp;
       }
      for (k=1;k<=i-1;k++) {	/* Text error: Only swap a few elements in L.*/
        temp    = L[p][k];	/* Matrix L.                                 */
        L[p][k] = L[i][k];
        L[i][k] = temp;
      }
  ...
  /*
  **  Steps 11 and 12 solve lower-triangular system LZ = B.
  */
  ...
  /*
  **  Steps 13 and 14 solve upper-triangular system UX=Z.
  */
