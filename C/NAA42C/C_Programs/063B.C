...
Exercise Set 6.3, Problems 1 (iii), 3-4.  For chopping arithmetic, replace
...
char *outfile = "063b.out";	/* Customized default output file name.     */


main()
{
  float **A, **E, **m, *s, *X, max, sum, temp;
  int i, j, k, n, p, SIG, *NROW, NCOPY;
  ...
  for (i=1;i<n;i++) {		/* Do Steps 3-6.  Elimination process.  */
    ...
    for (j=i;j<=n;j++) {	/* Find maximum element in column i.    */
				/* Different from Alg. 6.2.             */
      temp = round(fabs(A[NROW[j]][i]) / s[NROW[p]], SIG);
      if (temp > max) {
        max = temp;
        p = j;
      }
